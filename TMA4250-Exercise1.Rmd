...
set.seed(123)
x = seq(0,5,length.out = 1001) # distances are from 0 to 5
mat_1 = matern(x, 1, 1) # create correlation functions
mat_2 = matern(x, 1, 3)
...
```{r, echo = F, eval = T, out.width = "50%", fig.align = "center"}
gamma_1 = (1-mat_1) # Create variograms
gamma_2 = (1-mat_2)
gamma_3 = (1-pexp_1)
gamma_4 = (1-pexp_2)
...
x = seq(1,50) # Vector of x's
tau = abs(outer(x,x,'-')/10) # Matrix of distances
mu_r = rep(0,50) # Mean vector

sigmamatrix1 = matern(tau,1,1) # Covariance matrix
sample1 = mvrnorm(10,mu_r,sigmamatrix1) # Sample from prior distribution
...
sample = sample2[2,] # Pick one of the samples with sigma^2 = 5
sigma_r_rho = sigmamatrix2
observation = c(sample[10], sample[25], sample[30])
H = matrix(0, nrow = 3, ncol = 50) # Create the observation matrix H
H[1,10] = 1
H[2,25] = 1
H[3,30] = 1
sigma_e = 0
...
prediction_zero = apply(sample_zero,2,mean) # Find prediction by taking mean
std_empirical_zero = sqrt(apply(sample_zero,2,var)) # Find prediction variance
...
v2<-ggplot() + 
  ...
  xlab(expression(paste(tau))) + ylab("Variogram") + ggtitle("Comparison of empirical, estimated and \ntheoretical variogram with 64 observations") + theme(plot.title = element_text(size = 12))
...
v3<-ggplot() + 
  ...
  xlab(expression(paste(tau))) + ylab("Variogram") + ggtitle("Comparison of empirical, estimated and \ntheoretical variogram with 100 observations") + theme(plot.title = element_text(size = 12))
v1
v2
v3
```
