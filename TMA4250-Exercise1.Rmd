...
## b)
```{r, echo = F, eval = T}
vario <- variog(coords = dat, data = sample)
...
ggplot() + 
  ...
  xlab(expression(paste(tau))) + ylab("Variogram") + ggtitle("Comparison of empirical and theoretical variogram") + scale_color_manual(name="", values = c("black","blue"))
```
...
```{r, echo = F, eval = T}
n_obs = 36
x_loc <- ceiling(runif(n_obs,0,30))
y_loc <- ceiling(runif(n_obs,0,30))
coordinates = matrix(c(x_loc,y_loc), nrow = n_obs, ncol = 2)
datapoints = sample[x_loc+(y_loc-1)*30]
vario2 <- variog(coords = coordinates, data = datapoints)
...
```{r, echo = F, eval = T}
parameters <- likfit(coords = dat, data = sample, ini.cov.pars = c(1,10))
parameters2 <- likfit(coords = coordinates, data = datapoints, ini.cov.pars = c(1,10))
...
ggplot() + 
  ...
  geom_line(data = df, aes(y = y3, x = x, colour = "Partial")) +
  xlab(expression(paste(tau))) + ylab("Variogram") + ggtitle("Comparison of maxlik... and theoretical variogram") + scale_color_manual(name="", values = c("black","blue", "red"))
