...
-Is a stationary Gaussian RF a suitable model for the terrain elevation in domain D?
...
\begin{align*}
E\{r(\mathbf{x})\} &= \mu_r = 0\\
Var\{r(\mathbf{x})\} &= \sigma^2_r\\
Corr\{r(\mathbf{x}), r(\mathbf{x}')\} &= \exp\{-\tau/\xi_r\}
\end{align*}
with $\tau = |\mathbf{x} − \mathbf{x}'|$.

## a)
We discretize the Gaussian RF to get $\{r(x); x \in L\}$ on a grid $L : [30×30] \in D$. The model parameters are set to $\sigma^2_r = 2$ and $\xi_r = 15$. Then we generate one realization
of the discretized Gaussian RF and display it.
