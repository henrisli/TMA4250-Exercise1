...
subtitle: 'Group members: Henrik Syversveen Lie, Øyvind Auestad (SKRIVE STUDENTNUMMER
  I STEDET??)'
---
...
We can interpret the model such that mean is the level around which the elevation fluctuates, while the variance is the degree of fluctuation. The level is here a second order polynomial in the coordinates, while the prior variance is constant over the coordinates. 
...
If we change the parametrization of the expectation function, it could be natural also to change the value of the prior variance, $\sigma_r^2$. For example when we reduce the degree of the polynomial in the expectation function it could be that we might have to compensate with a larger prior variance to get bigger fluctutations around this new 'less flexible' mean. But in practice it would be more natural to estimate the prior variance using maximum likelihood, with our data points.

The kriging predictions looks very similar, but there are some differences, which become most clear in the corners of the field. The same is true for the kriging variances. It seems like it is the model with second order expectation which has the largest kriging variances. From these plots it is clear where the observations are located. Moving further away from the observations we see that the kriging variance increases. It always stays below the prior variance ($2500$), with the maximum value being just above $1600$, obtained in the bottom left corner of the first variance plot. 
...
image.plot(x = x, y = y, z = matrix(kriging_2nd_25$krige.var, nrow = 315), main = "Variance. First order mean function. \nMeasurement error variance = 25", breaks = var_brk, col = color_table)
```

The result is plotted above. Again, it is hard to see a clear difference in the plots of the kriging predictions, but it is possible if you look in the corners of the plot. The kriging variances differ more clearly. For example are we able to see how the measurement error variance increases the kriging variance. This is particularly clear in the plots of the variances of the model with first order expectation function and different measurement error variances. Also here the kriging variance takes on larger values for the model with second order expectation function. The largest being just below $1700$, obtained in the bottom left corner. The variances always stays below the prior variance, and tends towards this with increasing distance from the observations. 
...
v1<-ggplot() + 
  ...
  xlab(expression(paste(tau))) + ylab("Variogram") + ggtitle("Comparison of empirical, estimated and theoretical variogram with 9 observations") + theme(plot.title = element_text(size = 8))
...
v2<-ggplot() + 
  ...
  xlab(expression(paste(tau))) + ylab("Variogram") + ggtitle("Comparison of empirical, estimated and theoretical variogram with 64 observations") + theme(plot.title = element_text(size = 8))
...
v3<-ggplot() + 
  ...
  xlab(expression(paste(tau))) + ylab("Variogram") + ggtitle("Comparison of empirical, estimated and theoretical variogram with 100 observations") + theme(plot.title = element_text(size = 8))
v1
v2
v3
```
